...
const purchases = [
  { text: "Someone from India just bought 1000 Followers.", amount: 1000 },
  { text: "Someone from Canada just bought 500 Followers.", amount: 500 },
  { text: "Someone from India just bought 700 Followers.", amount: 700 },
  { text: "Someone from Canada just bought 2000 Followers.", amount: 2000 }
];

const DeliveredFollowersSection = () => {
  ...
  const [totalFollowers, setTotalFollowers] = useState(() => {
    return parseInt(localStorage.getItem("totalFollowers")) || 235000000; // Load from localStorage or set default value
  });
  const sectionRef = useRef(null);

  useEffect(() => {
    localStorage.setItem("totalFollowers", totalFollowers); // Update localStorage whenever totalFollowers changes
  }, [totalFollowers]);
  ...
  useEffect(() => {
    const purchaseInterval = setInterval(() => {
      ...
      setPurchaseIndex((prev) => {
        ...
        setTotalFollowers((prevLikes) => {
          const newTotal = prevLikes + purchases[nextIndex].amount;
          localStorage.setItem("totalFollowers", newTotal); // Save updated count to localStorage
          return newTotal;
        });
        return nextIndex;
      });
      ...
      <motion.div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <motion.div initial="hidden" animate={textControls} variants={fadeInLeft}>
          ...
          <p className="text-lg md:text-xl text-gray-300 mt-4">
            Get <strong>high-quality Instagram Followers</strong> from real users—no bots, no fake accounts.
        ...
        <div className="flex flex-col items-center md:items-end gap-6 relative">
          <motion.div initial="hidden" animate={statsControls} variants={fadeInRight} className="bg-white/10 backdrop-blur-lg border border-white/20 p-6 md:p-8 rounded-2xl shadow-lg flex flex-col items-center gap-4 relative">
            ...
            <div className="text-center">
              ...
              <p className="text-lg text-gray-300">Followers delivered worldwide</p>
            </div>
            {/* Popup Notification */}
            <AnimatePresence>
              {showPurchase && (
...
export default DeliveredFollowersSection;
