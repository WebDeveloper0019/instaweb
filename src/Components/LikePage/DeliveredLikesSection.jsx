...
const DeliveredLikesSection = () => {
  ...
  const [totalLikes, setTotalLikes] = useState(() => {
    return parseInt(localStorage.getItem("totalLikes")) || 2350000000; // Load from localStorage or set default value
  });
      ...
      <motion.div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <motion.div initial="hidden" animate={textControls} variants={fadeInLeft}>
          ...
          <p className="text-lg md:text-xl text-gray-300 mt-4">
            Get <strong>high-quality Instagram likes</strong> from real users—no bots, no fake accounts.
        ...
        <div className="flex flex-col items-center md:items-end gap-6 relative">
          <motion.div initial="hidden" animate={statsControls} variants={fadeInRight} className="bg-white/10 backdrop-blur-lg border border-white/20 p-6 md:p-8 rounded-2xl shadow-lg flex flex-col items-center gap-4 relative">
            <FaHeart className="text-red-500 text-5xl animate-pulse" />
            <div className="text-center">
              <p className="text-5xl font-bold text-white">{totalLikes.toLocaleString()}</p>
              <p className="text-lg text-gray-300">Likes delivered worldwide</p>
            </div>
            {/* Popup Notification */}
            <AnimatePresence>
              {showPurchase && (
                <motion.div
                  ...
                  className="absolute top-0 right-32 transform translate-x-full bg-white text-gray-900 p-4 rounded-lg shadow-lg flex items-center space-x-3 border border-gray-200 z-50"
                >
...
export default DeliveredLikesSection;
