...
const purchases = [
  { text: "Someone from India just bought 1000 Views.", amount: 1000 },
  { text: "Someone from Canada just bought 500 Views.", amount: 500 },
  { text: "Someone from India just bought 700 Views.", amount: 700 },
  { text: "Someone from Canada just bought 2000 Views.", amount: 2000 }
];
...
const ViewDeliveredLikesSection = () => {
  ...
  const [totalViews, setTotalViews] = useState(() => {
    return parseInt(localStorage.getItem("totalViews")) || 235000000; // Load from localStorage or set default value
  });
  const sectionRef = useRef(null);

  useEffect(() => {
    localStorage.setItem("totalViews", totalViews); // Update localStorage whenever totalViews changes
  }, [totalViews]);
  ...
  useEffect(() => {
    const purchaseInterval = setInterval(() => {
      ...
      setPurchaseIndex((prev) => {
        ...
        setTotalViews((prevLikes) => {
          const newTotal = prevLikes + purchases[nextIndex].amount;
          localStorage.setItem("totalViews", newTotal); // Save updated count to localStorage
          return newTotal;
        });
        return nextIndex;
      });
      ...
      <motion.div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
        ...
        <motion.div initial="hidden" animate={textControls} variants={fadeInLeft}>
          ...
          <p className="text-lg md:text-xl text-gray-300 mt-4">
            Get <strong>high-quality Instagram views</strong> from real users—no bots, no fake accounts.
        ...
        <div className="flex flex-col items-center md:items-end gap-6 relative">
          <motion.div initial="hidden" animate={statsControls} variants={fadeInRight} className="bg-white/10 backdrop-blur-lg border border-white/20 p-6 md:p-8 rounded-2xl shadow-lg flex flex-col items-center gap-4 relative">
            ...
            <div className="text-center">
              <p className="text-5xl font-bold text-white">{totalViews.toLocaleString()}</p>
              <p className="text-lg text-gray-300">Views delivered worldwide</p>
            </div>
            {/* Popup Notification */}
            <AnimatePresence>
              {showPurchase && (
