...
const ViewPricingSlider = () => {
  ...
  const handleDropdownChange = (event) => {
    const index = viewsPlans.findIndex((option) => option.amount === parseInt(event.target.value, 10));
    if (index !== -1) {
      setSelectedIndex(index);
    }
  };

  const selectedPlan = viewsPlans[selectedIndex];
  ...
  return (
    <section className="py-16 px-6 md:px-12 lg:px-24 flex flex-col items-center text-white bg-transparent rounded-3xl">
      ...
      <div className="bg-white shadow-2xl rounded-xl p-8 pt-12 w-full max-w-lg mt-8 text-gray-900 relative">
        ...
        >
          <FaEye className="text-2xl" /> {selectedPlan.amount} Views
        </motion.div>
...
export default ViewPricingSlider;